...
from equi_diffpo.policy.diffusion_unet_hybrid_image_policy import (
...
from mirrorduo.augmentor.robo_augmentor import RoboAugmentor

OmegaConf.register_new_resolver("eval", eval, replace=True)


class TrainRealDiffusionUnetHybridWorkspace(BaseWorkspace):
    ...
    def __init__(self, cfg: OmegaConf, output_dir=None):
        ...
        if "robo_augmentor" in cfg:
            cfg_dict = OmegaConf.to_container(cfg, resolve=True)
            self.robo_augmentor = RoboAugmentor(
                shape_meta=cfg_dict["shape_meta"],
                configs=cfg_dict["robo_augmentor"],
            )
            ...
            while self.epoch < cfg.training.num_epochs:
                ...
                with tqdm.tqdm(
                    ...
                    for batch_idx, batch in enumerate(tepoch):
                        ...
                        # pass batches through the augmentor, it does nothing if not enabled
                        self.robo_augmentor(batch, self.epoch)
                ...
                if (self.epoch % cfg.training.checkpoint_every) == 0:
                    ...
                    print(f"Saving checkpoint to {topk_ckpt_path}")
...
def main(cfg):
    workspace = TrainRealDiffusionUnetHybridWorkspace(cfg)
    workspace.run()
