...
class RobomimicReplayMirrorDuoImageDataset(BaseImageDataset):
    def __init__(
        self,
        shape_meta: dict,
        dataset_path: str,
        ...
        self.all_action_keys.remove("action")
            ...
            with FileLock(cache_lock_path):
                if not os.path.exists(cache_zarr_path):
                    ...
                    try:
                        ...
                        replay_buffer = self.convert_robomimic_to_replay(
    ...
    def convert_action_to_xyz_6d_gripper(raw_actions, rotation_transformer):
        ...
        xyz = raw_actions[..., :3]  # x y z
        rot = raw_actions[..., 3:6]  # axis angle (scaled for relative actions)
        gripper = raw_actions[..., 6:]

        rot_6d = rotation_transformer.forward(rot)
        act_6d_xyz_gripper = np.concatenate([xyz, rot_6d, gripper], axis=-1)

        return act_6d_xyz_gripper.astype(np.float32)
    ...
    def convert_robomimic_to_replay(
        ...
        with h5py.File(dataset_path) as file:
            ...
            for i in range(n_demo):
                ...
                episode_length = demo["delta_actions"].shape[0]
            ...
            for key in tqdm(lowdim_keys + self.all_action_keys, desc="Loading lowdim data"):
                ...
                target_key = key
                if "_delta" in key:
                    # delta proprioception, revert to original key for loading
                ...
                if data_key not in demos["demo_0"]:
                    ...
                    if key in self.all_action_keys:
                        this_data = self.convert_action_to_xyz_6d_gripper(
                            raw_actions=this_data, rotation_transformer=rotation_transformer
                        )
                        assert this_data.shape == (n_steps,) + tuple(shape_meta["action"]["shape"])
                    else:
                        if is_matched_key("pos", key, reject_pattern="delta"):
                            create_array(
                                data_group,
                                key.replace("eef_pos", "eef_delta_pos"),
                                get_delta_positions(this_data),
                            )
                        elif is_matched_key("rot", key, reject_pattern="delta"):
                            is_6d = shape_meta["obs"][target_key]["shape"] == [6]
                            create_array(
                                data_group,
                                key.replace("eef_rot", "eef_delta_rot"),
                                get_delta_rotations(this_data, return_6d=is_6d),
                            )
                            # only keep 6d, i.e. the first two rows of the rot matrix
                            this_data = this_data[..., :6] if is_6d else this_data
